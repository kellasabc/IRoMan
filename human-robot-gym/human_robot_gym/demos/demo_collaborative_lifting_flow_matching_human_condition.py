"""Demo script for the collaborative lifting environment using Flow Matching model with Human Action Condition.
Uses a trained Flow Matching model with human action as condition to control the robot.

Flow Matching model output description (Human Action Condition):
- Input: 23-dimensional observation state (obs_keys) + 6-dimensional human action condition (past_action)
...
Author:
    Modified from demo_collaborative_lifting_flow_matching.py to use Flow Matching model with human action condition
"""
...
class FlowMatchingHumanConditionAgent:
    """Flow Matching agent with human action condition for collaborative lifting task"""
    ...
    def __init__(self, model_path, device="gpu", robot_action_dim=4, human_action_dim=6):
        ...
        # Load checkpoint
        print(f"Loading Flow Matching model with human condition from: {model_path}")
        ...
        # Force enable human action condition
        cfg.human_act_as_cond = True
        cfg.policy.human_act_as_cond = True
        if hasattr(cfg, 'task') and hasattr(cfg.task, 'dataset'):
            cfg.task.dataset.human_act_as_cond = True
        
        print("✓ Enabled human action condition")
        ...
        print("✓ Flow Matching model with human condition loaded successfully")
    ...
    def predict_action(self, observation, human_action=None):
        """
        Predict action from observation with human action condition
        Args:
            observation: 23-dimensional observation array
            human_action: 6-dimensional human action array (optional, will use zeros if not provided)
        Returns:
        ...
        # Update past_action buffer with current human action
...
if __name__ == "__main__":
    ...
    def switch_agent():
        ...
        if use_kb_agent:
            ...
            print("Switched to Flow Matching model with human condition control")
    ...
    # Load Flow Matching model with human condition
    model_path = os.path.join(workspace_root, "flow_copolicy", "outputs", "flow_matching_collaborative_lifting_human_cond", "best_model.ckpt")
    
    # Check if best_model.ckpt exists, otherwise try to find the latest checkpoint
    if not os.path.exists(model_path):
        checkpoints_dir = os.path.join(workspace_root, "flow_copolicy", "outputs", "flow_matching_collaborative_lifting_human_cond", "checkpoints")
    ...
    # Initialize Flow Matching agent with human condition
    device = "cuda" if torch.cuda.is_available() else "cpu"
    print(f"Using device: {device}")
    
    flow_matching_agent = FlowMatchingHumanConditionAgent(
    ...
    for i_episode in range(20):
        ...
        while True:
            ...
            if use_kb_agent:
                ...
                # Use Flow Matching model with human condition
                ...
                # Get action from Flow Matching model with human condition
                planning_start = time.time()
                robot_action = flow_matching_agent.predict_action(obs_23d, human_action)
    ...
    if planning_times:
        ...
        print("\n" + "="*60)
        print("🎯 Flow Matching Human Condition Model Planning Time Statistics Summary")
        print("="*60)
