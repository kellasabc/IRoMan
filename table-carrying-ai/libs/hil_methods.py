...
from libs.planner.planner_utils import (is_safe, pid_single_step, tf2model,
                                        tf2sim, update_queue)
# Add the diffusion_policy path relative to current workspace
import os
current_dir = os.path.dirname(os.path.abspath(__file__))
workspace_root = os.path.join(current_dir, '..', '..')
diffusion_policy_path = os.path.join(workspace_root, 'diffusion_copolicy', 'diffusion_policy')
...
def play_hil_planner(
    ...
    if robot == "planner":
        ...
        if planner_type == "vrnn":
            ...
            # Define workspace root for path resolution
            import os
            current_dir = os.path.dirname(os.path.abspath(__file__))
            workspace_root = os.path.join(current_dir, '..', '..')
            artifact_path = join(os.path.join(workspace_root, "table-carrying-ai/trained_models/vrnn/model.ckpt"))
            ...
            if mcfg.human_act_as_cond:
                ...
                ckpt_path = join(os.path.join(workspace_root,
                                              "table-carrying-ai/trained_models/diffusion/model_human_act_as_cond_10Hz.ckpt"))
            else:
                # ckpt_path = join("/home/eleyng/table-carrying-ai/trained_models/diffusion/model_10Hz.ckpt")
                ckpt_path = join(
                    os.path.join(workspace_root, "table-carrying-ai/trained_models/diffusion/model_10Hz.ckpt"))
            ...
            dataset_path = os.path.join(workspace_root, "diffusion_copolicy/data/table/table_10Hz.zarr")
    ...
    if planner_type in ["diffusion_policy", "bc_lstm_gmm"]:
        ...
        past_action = np.zeros((3, 4)) if planner_type == "diffusion_policy" else np.zeros_like(obs.squeeze())[..., :4]
                ...
                if robot == "real" and exp_run_mode == "hil": # Turing Test mode
                    ...
                    if planner_type == "vrnn":
                        ...
                        if planner_type == "diffusion_policy":
                            ...
                            current_dir = os.path.dirname(os.path.abspath(__file__))
                            workspace_root = os.path.join(current_dir, '..', '..')
                            diffusion_policy_path = os.path.join(workspace_root, 'diffusion_copolicy',
                                                                 'diffusion_policy')
                            diffusion_copolicy_path = os.path.join(workspace_root, 'diffusion_copolicy')
                            sys.path.append(diffusion_policy_path)
                            sys.path.append(workspace_root)
