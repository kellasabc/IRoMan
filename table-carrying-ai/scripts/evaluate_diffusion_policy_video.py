#!/usr/bin/env python3
"""
Diffusion Policy Evaluation Script for Table Carrying AI with Visualization

This script performs evaluation of trained diffusion policy models with
visualization and video recording capabilities.

Usage:
    python evaluate_diffusion_policy_visual.py --run-mode [coplanning | replay_traj | hil] \
...
class DiffusionPolicyVisualEvaluator:
    """Diffusion Policy Evaluator with visualization and video recording capabilities"""
    ...
    def create_environment(self, map_file, episode_name):
        ...
        if self.args.record_video:
            ...
            render_mode = self.args.render_mode  # 使用用户指定的渲染模式
    ...
    def record_video_from_env(self, env, episode_name, max_frames=1000):
        """Record video from environment using manual frame capture"""
        video_path = join(self.video_dir, f"{episode_name}.mp4")
        print(f"Recording video to: {video_path}")

        # Get first frame to determine dimensions
        frame = env.render(mode="rgb_array")
        if frame is None:
            print("Warning: Could not render frame, skipping video recording")
            return None
        ...
        if not video_writer.isOpened():
            print("Warning: Could not open video writer, skipping video recording")
            return None
    ...
    def run_single_evaluation(self, env, collision_checking_env, playback_trajectory, episode_name):
        ...
        video_writer = None
        if self.args.record_video:
            video_writer = self.record_video_from_env(env, episode_name)

        # Run evaluation using hil_methods_diffusion (without video_path parameter)
    ...
    def run_evaluation(self):
        ...
        print("Starting diffusion policy evaluation with visualization...")
...
def main():
    parser = argparse.ArgumentParser("Diffusion Policy Evaluation for Table Carrying AI with Visualization")
    ...
    evaluator = DiffusionPolicyVisualEvaluator(args)
