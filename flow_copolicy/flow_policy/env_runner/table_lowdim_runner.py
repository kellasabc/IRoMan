...
from flow_policy.env.cooperative_transport.gym_table.envs.table_env import \
    TableEnv
from flow_policy.env.cooperative_transport.gym_table.envs.utils import \
    CONST_DT
from flow_policy.env_runner.base_lowdim_runner import BaseLowdimRunner
from flow_policy.gym_util.async_vector_env import AsyncVectorEnv
from flow_policy.gym_util.multistep_wrapper import MultiStepWrapper
from flow_policy.gym_util.sync_vector_env import SyncVectorEnv
...
class TableLowdimRunner(BaseLowdimRunner):
    ...
    def run(self, policy: BaseLowdimPolicy):
        ...
        for chunk_idx in range(n_chunks):
            ...
            obs = env.reset()
            env.ep = self.env_ep
            # 修复 past_action 的形状，使其能够存储 n_obs_steps 个动作
            past_action = np.zeros((obs.shape[0], self.n_obs_steps, 4))
            policy.reset()
            ...
            while not done:
                # create obs dict - 观察数据完全从数据集中读取，不进行修改
                ...
                # 取第一个动作步，因为 action 的形状是 (n_envs, n_action_steps, action_dim)
