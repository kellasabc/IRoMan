...
from flow_policy.model.common.dict_of_tensor_mixin import DictOfTensorMixin
...
class LinearNormalizer(DictOfTensorMixin):
    ...
    def fit(self,
        ...
        if isinstance(data, dict):
            for key, value in data.items():
                ...
                # 将fitted_params的所有参数添加到self.params_dict中
                for param_key, param_value in fitted_params.items():
                    param_name = f"{key}_{param_key}"
                    self.params_dict[param_name] = param_value
        else:
            fitted_params = _fit(data, 
                    last_n_dims=last_n_dims,
                    dtype=dtype,
                    mode=mode,
                    output_max=output_max,
            ...
            # 将fitted_params的所有参数添加到self.params_dict中
            for param_key, param_value in fitted_params.items():
                param_name = f"_default_{param_key}"
                self.params_dict[param_name] = param_value
    ...
    def __getitem__(self, key: str):
        ...
        for param_key, param_value in self.params_dict.items():
            if param_key.startswith(f"{key}_"):
                actual_param_key = param_key[len(f"{key}_"):]
                params[actual_param_key] = param_value
    ...
    def _normalize_impl(self, x, forward=True):
        if isinstance(x, dict):
            ...
            params = nn.ParameterDict()
            for param_key, param_value in self.params_dict.items():
                if param_key.startswith("_default_"):
                    actual_param_key = param_key[len("_default_"):]
                    params[actual_param_key] = param_value
